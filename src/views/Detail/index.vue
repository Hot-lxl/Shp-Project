...
export default {
  ...
  methods: {
    // 点击的属性高亮（使用排他思想）
    isChecked(spuSaleAttrValue, arr, spuSaleAttr) {
      //  先让所有的属性都变为0（不显示）
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      ...
      // 判断属性属于那个
      if (spuSaleAttr.saleAttrName == "颜色") {
        // 把属性值赋值给定义的变量
        this.colorAttr = spuSaleAttrValue.saleAttrValueName;
      }
    },
    //表单元素修改产品个数
    changeSkuNum(event) {
    ...
    async addToCart() {
      ...
      try {
        ...
        // 如果是成功则路由跳转并且需要商品的id以及商品详情信息
        // 一些简单的数据skuNum，colorAttr，通过query形式传给路由组件
        // 而一些复杂的数据（产品信息：skuinfo），可以通过会话存储（不持久化，会话结束数据就会消失）
        // 不论本地存储还是会话存储都不能传对象
        sessionStorage.setItem("SKUINFO", JSON.stringify(this.skuInfo));
        this.$router.push({
          name: "addcartsuccess",
          query: { skuNum: this.skuNumber, colorAttr: this.colorAttr },
        });
      } catch (error) {
        //  失败
        alert(error.message);
